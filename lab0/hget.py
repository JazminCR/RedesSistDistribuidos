...
def connect_to_server(server_name):
    ...
    # Crear socket
    # COMPLETAR ABAJO DE ESTA LINEA
    # creacion de socket: AF_INET (Internet protocol IPv4) & SOCK_STREAM (TCP)
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  
    # conexion con el puerto 80 (http)
    s.connect((ip_address, HTTP_PORT))                    
    # Aqui deben conectarse al puerto correcto del servidor
    # devolver el socket para luego poder comunicarse con el servidor
    return s                                              
